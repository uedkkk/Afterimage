import { getAllSettings, getPublishedAlbums, getAllCategories } from "@/lib/db/queries";
...
export async function Footer() {
  ...
  const footerAlbums = albums.slice(0, 4);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
          <div>
            ...
            <FooterLink href="/">作品集</FooterLink>
            <FooterLink href="/stories">影像故事</FooterLink>
            <FooterLink href="/about">关于</FooterLink>
            <FooterLink href="/search">搜索</FooterLink>
          </div>
          <div>
            <h4 className="text-[13px] font-bold uppercase tracking-[0.04em] text-white/50 mb-4">
              影集
            </h4>
            {footerAlbums.length > 0 ? (
              footerAlbums.map((album) => (
                <FooterLink key={album.id} href={`/album/${album.slug}`}>
                  {album.title}
                </FooterLink>
              ))
            ) : (
              <FooterLink href="/search">浏览全部</FooterLink>
            )}
          </div>
          <div>
            <h4 className="text-[13px] font-bold uppercase tracking-[0.04em] text-white/50 mb-4">
            ...
            {categories.length > 0 ? (
              categories.slice(0, 4).map((cat) => (
                <FooterLink key={cat.id} href={`/category/${cat.slug}`}>
                  {cat.name}
                </FooterLink>
              ))
            ) : (
              <>
                <FooterLink href="/category/travel">旅行</FooterLink>
                <FooterLink href="/category/city">城市</FooterLink>
                <FooterLink href="/category/street">街拍</FooterLink>
                <FooterLink href="/category/portrait">人像</FooterLink>
              </>
            )}
          </div>
          <div>
            ...
            <FooterLink href="https://xiaohongshu.com" external>小红书</FooterLink>
            <FooterLink href="/rss" external>RSS</FooterLink>
          </div>
        </div>
