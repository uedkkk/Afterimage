...
export function AlbumCard({ album }: AlbumCardProps) {
  return (
    <Link
      ...
      className="relative overflow-hidden cursor-pointer bg-dust group block aspect-[4/3] rounded-stadium shadow-card transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 hover:shadow-drama no-underline"
    >
      {album.cover && (
        <Image
      ...
      <div className="absolute inset-0 p-5 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-black/70 to-transparent text-white">
        ...
        {album.description && (
          <p className="text-white/70 text-[14px] font-450 mt-1 line-clamp-2">
            {album.description}
          </p>
        )}
      </div>
